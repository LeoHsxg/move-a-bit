// 動一下 — shared design system: tokens, icons, primitives
// Warm, minimalist, low-pressure. No red, no competitive elements.

const C = {
  bg:       '#FBF7EF', // warm white screen base
  bgWarm:   '#F5ECDC', // alt warm panel
  cream:    '#F6EFE1',
  card:     '#FFFFFF',
  clay:     '#D89A72', // primary terracotta (actions)
  clayDeep: '#C07F54',
  claySoft: '#F1DECB', // light terracotta fill
  clayWash: '#FBEEE2',
  sage:     '#8BA47E', // nature accent (印記)
  sageDeep: '#6E8A62',
  sageSoft: '#DEE7D1',
  sand:     '#ECDFC8',
  ink:      '#4A443C', // primary text (warm near-black)
  ink2:     '#776E61', // secondary
  ink3:     '#A89E8E', // muted
  line:     'rgba(74,68,60,0.10)',
  lineSoft: 'rgba(74,68,60,0.055)',
};

const FZ = '"Noto Sans TC", -apple-system, system-ui, sans-serif';
const FN = '"Quicksand", "Noto Sans TC", system-ui, sans-serif'; // rounded display/numbers

const PHONE_W = 390;
const PHONE_H = 844;

// ── Status bar (warm, dark glyphs) ───────────────────────────
...
// ── Screen shell — fills the 390×844 artboard ────────────────
function Screen({ children, bg = C.bg, light = false, time = '9:41', noStatus = false }) {
  return (
    <div style={{
...
// ── Buttons ──────────────────────────────────────────────────
function Btn({ children, kind = 'primary', full = true, disabled = false, style = {} }) {
  const base = {
...
// ── Progress segments for onboarding ─────────────────────────
function Progress({ step, total = 5, back = true }) {
  return (
...
// ── Selection card (onboarding goals / location) ─────────────
function ChoiceCard({ title, sub, selected = false, leading, trailing, style = {} }) {
  return (
    <div style={{
...
// ── Chip (multi-select) ──────────────────────────────────────
function Chip({ children, selected = false }) {
  return (
    <div style={{
...
Object.assign(window, {
  C, FZ, FN, PHONE_W, PHONE_H,
  StatusBar, HomeBar, Screen, Btn, Progress, TabBar, ChoiceCard, Chip, Icon,
});
